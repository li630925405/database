...
import WishProfile from "./wish_profile";
...
class PlayTable extends PureComponent {
	...
	componentDidMount = () => {
		...
		let url = 'http://localhost:8000/list_Dreamlist/';
	...
	extractGameInfo = () => {
		...
		let l2 = this.state.game_names.length;
		if (l1 > 0 && l2 > 0) {
			for (let i = 0; i < l1; i++) {
				for (let j = 0; j < l2; j++) {
					game = this.state.all_games[i];
					name = this.state.game_names[j].fields.game;
				...
				{(_data || []).map((item, index) => (
					<Table.Row>
							<Table.Cell>
								...
								<WishProfile type={item.fields.type} name={item.fields.name} price={item.fields.price} grade={item.fields.grade}
 								pubtime={item.fields.pubtime} avatar={item.fields.avatar} n_comments={item.fields.n_comments} />
 								</Table.Cell>
					</Table.Row>
				))}
			</Table.Body>
